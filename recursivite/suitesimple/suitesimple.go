...
/*
On considère la suite dont le terme u(n) est défini par u(n) = (17 * u(n-1)) % 13 et telle que u(0)=1. La fonction terme calcule
...
func recur(n, count, value int) int {
	if count == n {
		return value
	} else {
		return recur(n, count+1, (value*17)%13)
	}
}
