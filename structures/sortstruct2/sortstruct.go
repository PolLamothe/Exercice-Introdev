...
func sortByMarkThenLastNameThenFirstName(t []student) {
	for i := 0; i < len(t); i++ {
		for j := i + 1; j < len(t); j++ {
			if t[i].mark < t[j].mark {
				t[i], t[j] = t[j], t[i]
				sortByMarkThenLastNameThenFirstName(t)
				return
			} else if t[i].mark == t[j].mark {
				if t[i].lastName > t[j].lastName {
					t[i], t[j] = t[j], t[i]
					sortByMarkThenLastNameThenFirstName(t)
					return
				} else if t[i].lastName == t[j].lastName {
					if t[i].firstName > t[j].firstName {
						t[i], t[j] = t[j], t[i]
						sortByMarkThenLastNameThenFirstName(t)
						return
					}
